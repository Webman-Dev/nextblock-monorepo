-- 20260120120000_update_sandbox_reset.sql
-- Updates the reset_sandbox() function to include E-commerce pages AND the Premium Product.

CREATE OR REPLACE FUNCTION public.reset_sandbox(p_include_premium BOOLEAN DEFAULT FALSE)
RETURNS void
LANGUAGE plpgsql
...
DECLARE
  ...
  v_shop_page_id bigint;
  v_cart_page_id bigint;
  v_checkout_page_id bigint;
...
BEGIN
  ...
  TRUNCATE TABLE 
    ...
    public.logos,
    public.products,      -- Clear products
    public.product_media, -- Clear product media
    public.orders,        -- Clear orders
    public.order_items    -- Clear order items
  ...
  -- 4b. Pages (Premium Store)
  IF p_include_premium THEN
    INSERT INTO public.pages (language_id, title, slug, status, translation_group_id)
    VALUES 
      (v_en_lang_id, 'Shop', 'shop', 'published', v_shop_page_group_id),
      (v_en_lang_id, 'Cart', 'cart', 'published', v_cart_page_group_id),
  ...
  IF p_include_premium THEN
    SELECT id INTO v_shop_page_id FROM public.pages WHERE slug = 'shop' AND language_id = v_en_lang_id;
    SELECT id INTO v_cart_page_id FROM public.pages WHERE slug = 'cart' AND language_id = v_en_lang_id;
    SELECT id INTO v_checkout_page_id FROM public.pages WHERE slug = 'checkout' AND language_id = v_en_lang_id;
...
<p>Chaque choix architectural se renforce : Nx garde les dépendances lisibles, l'app Next.js impose une UX cohérente, les migrations Supabase codifient l'accès, et l'éditeur Tiptap assure une expérience identique pour tous les contributeurs.</p>$$
  ...
  -- INSERT: Blog (EN) and French Content Blocks
  INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
  VALUES
    (
      v_blog_page_id,
  ...
  -- 8b. Blocks (Premium Store)
  IF p_include_premium THEN
    -- Shop (EN)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
    VALUES (v_shop_page_id, v_en_lang_id, 'product_grid', '{"type": "latest"}'::jsonb, 0);
    -- Cart (EN)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
    VALUES (v_cart_page_id, v_en_lang_id, 'cart', '{}'::jsonb, 0);
    -- Checkout (EN)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
    VALUES (v_checkout_page_id, v_en_lang_id, 'checkout', '{}'::jsonb, 0);

    -- Shop (FR)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
    VALUES (v_shop_page_fr_id, v_fr_lang_id, 'product_grid', '{"type": "latest"}'::jsonb, 0);
    -- Cart (FR)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
    VALUES (v_cart_page_fr_id, v_fr_lang_id, 'cart', '{}'::jsonb, 0);
    -- Checkout (FR)
    INSERT INTO public.blocks (page_id, language_id, block_type, content, "order")
  ...
  IF p_include_premium THEN
    INSERT INTO public.navigation_items (language_id, menu_key, label, url, "order", page_id)
    VALUES
        (v_en_lang_id, 'HEADER', 'Shop', '/shop', 2, v_shop_page_id),
        (v_fr_lang_id, 'HEADER', 'Boutique', '/boutique', 2, v_shop_page_fr_id);
  END IF;

  -- 10. PRODUCT SEEDING (Premium Only)
  IF p_include_premium THEN
      -- Insert Product
      INSERT INTO public.products (id, sku, title, slug, price, status, short_description, description_json, stock)
      VALUES (
        v_premium_product_id,
        'NB-PREMIUM-001',
        'NextBlock CMS E-commerce Premium',
        'nextblock-cms-ecommerce-premium',
        15000,
        'active',
        '1-Year License for NextBlock Premium features (GitHub Invitation).',
        '{"type": "doc", "content": [{"type": "paragraph", "content": [{"type": "text", "text": "NextBlock CMS E-commerce Premium. E-commerce is just the beginning and more projects are coming. This 1-year license (invitation through GitHub) grants you access to the premium repository features for $150/year. Support the development of the ultimate Next.js CMS."}]}]}'::jsonb,
        1000
      );
...
-- Auto-Execute for Local Dev (Assumes Premium context since this file exists here)
SELECT public.reset_sandbox(true);
