// libs/db/src/supabase/functions/nextblock-migration-ingest/index.ts
...
async function processMediaUpload(url, r2Config, warnings) {
  ...
  try {
    ...
    if (url.startsWith("data:")) {
      ...
      const extension2 = contentType.split("/")[1] || "jpg";
      filename = `upload_${crypto.randomUUID()}.${extension2}`;
    } else {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userEmailMap = /* @__PURE__ */ new Map();
    if (allUsers?.users) {
      allUsers.users.forEach((u) => userEmailMap.set(u.email, u.id));
    }
    ...
    if (entity_type === "site_logo") {
      for (const item of data) {
        ...
        if (logoMediaId) {
          const { data: existingLogo } = await supabaseClient.from("logos").select("id").limit(1).maybeSingle();
          if (existingLogo) {
            await supabaseClient.from("logos").update({ media_id: logoMediaId, name: "Site Logo" }).eq("id", existingLogo.id);
          } else {
      ...
      for (const item of data) {
        ...
        if (item.addresses && item.addresses.length > 0) {
          const mappedAddresses = item.addresses.map((addr) => ({
      ...
      const allSkusToFetch = /* @__PURE__ */ new Set();
      for (const item of data) {
        if (item.order_items) {
      ...
      const skuToProductId = /* @__PURE__ */ new Map();
      if (allSkusToFetch.size > 0) {
        const { data: matchedProducts } = await supabaseClient.from("products").select("id, sku").in("sku", Array.from(allSkusToFetch));
        if (matchedProducts) {
      ...
      for (const item of data) {
        ...
        let query = supabaseClient.from("orders").select("id").eq("created_at", item.created_at).eq("total", item.total);
        if (userId) {
      ...
      const validMenuKeys = ["HEADER", "FOOTER", "SIDEBAR"];
      const selectedMenus = /* @__PURE__ */ new Map();
      const resolveMenuKey = (item) => {
        const rawKey = String(item.nextblock_key || item.menu_key || item.slug || "").toUpperCase();
        if (validMenuKeys.includes(rawKey)) {
          return rawKey;
        }
      ...
      const resolveLanguageId = (languageCode) => {
      ...
      const menuKeysToClear = /* @__PURE__ */ new Set();
      ...
      for (const { item, menuKey, languageCode, languageId } of selectedMenus.values()) {
        const insertMenuNode = async (node, parent_id, orderIdx) => {
      ...
      if (entity_type === "post" || entity_type === "page") {
        ...
        const uniqueEntitiesMap = /* @__PURE__ */ new Map();
        for (const entity of processedEntities) {
          if (!entity.slug) {
            entity.slug = `untitled-${Date.now()}-${Math.floor(Math.random() * 1e3)}`;
          }
        ...
        const { data: upsertedData, error: upsertErr } = await supabaseClient.from(tableName).upsert(finalEntitiesToUpsert, { onConflict: "language_id, slug" }).select("id, slug, language_id");
        if (upsertErr) {
        ...
        if (upsertedData) {
          ...
          const upsertedIds = upsertedData.map((u) => u.id);
          ...
          for (const item of data) {
            ...
            const dbRec = upsertedData.find((u) => u.slug === item.slug && u.language_id === langId);
            if (!dbRec) continue;
            let blockOrder = 0;
        ...
        const featureImageMap = /* @__PURE__ */ new Map();
        ...
        const { data: upsertedProducts, error: prodErr } = await supabaseClient.from("products").upsert(finalEntities, { onConflict: "language_id, slug" }).select("id, slug, language_id");
        if (prodErr) {
