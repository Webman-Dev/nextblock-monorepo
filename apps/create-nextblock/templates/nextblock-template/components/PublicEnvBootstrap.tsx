...
// /setup wizard then writes the env at runtime. The browser Supabase client + env checks
// read `process.env.NEXT_PUBLIC_* || window.__NEXTBLOCK_PUBLIC_ENV__`, so this fills the
// gap without a dev-server restart. In production the inlined values win and these props
// just match them (a harmless no-op). The url + anon key are public values (safe to ship).
//
...
export function PublicEnvBootstrap({ url, anonKey }: PublicEnv) {
  if (typeof window !== 'undefined' && url && anonKey) {
    window.__NEXTBLOCK_PUBLIC_ENV__ = { url, anonKey };
  }
  return null;
}
