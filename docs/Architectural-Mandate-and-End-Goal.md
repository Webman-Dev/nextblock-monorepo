...
3. **Publish Individually:** We must be able to publish libs/ui and libs/db as standalone NPM packages (@nextblock-monorepo/ui, @nextblock-monorepo/db) so they can be consumed by end-users without exposing the entire monorepo source code.

## **2\. The Distribution Strategy: create-nextblock**

**The End Goal:** A user runs npx create-nextblock my-app and gets a clean, standalone Next.js application.
...
2. Installs the Core Libraries (@nextblock-monorepo/db, @nextblock-monorepo/ui) as **dependencies** from NPM (or a local registry during dev).  
...
| **apps/create-nextblock** | App/CLI | create-nextblock | The CLI tool. Contains the templates/nextblock-template folder which is the *actual* product users receive. |
| **libs/db** | Lib | @nextblock-monorepo/db | **Supabase Layer.** Contains the client, server clients, and **Migrations** (src/supabase/migrations). |
| **libs/ui** | Lib | @nextblock-monorepo/ui | **Design System.** Shadcn/UI components, Tailwind configurations, and reusable UI elements. |
| **libs/editor** | Lib | @nextblock-monorepo/editor | **Tiptap Editor.** The block-based rich text editor logic. |
| **libs/sdk** | Lib | @nextblock-monorepo/sdk | **Developer SDK.** For building 3rd party blocks/extensions. |
| **libs/utils** | Lib | @nextblock-monorepo/utils | Shared helpers (cn, formatting, etc). |
...
* **In Monorepo (Dev):** apps/nextblock imports libs/ui via **TypeScript Path Aliases** (defined in tsconfig.base.json).  
  * *Example:* import { Button } from '@nextblock-monorepo/ui' \-\> resolves to libs/ui/src/index.ts.  
* **In Production (User):** The user's app imports libs/ui via **node\_modules**.  
  * *Example:* import { Button } from '@nextblock-monorepo/ui' \-\> resolves to node\_modules/@nextblock-monorepo/ui/dist/index.js.
...
* **Issue:** When installed as an NPM package, the file paths to SQL migrations inside node\_modules/@nextblock-monorepo/db are different than in the monorepo.  
* **Goal:** Ensure the DB package exports a utility or script that allows the consuming app to run migrations successfully, locating the .sql files correctly within node\_modules.

#### **2\. UI & Tailwind (libs/ui)**

* **Issue:** Styles missing in the consumer app.  
* **Goal:** Ensure libs/ui is transpiled correctly. The consumer app's tailwind.config.js must effectively scan node\_modules/@nextblock-monorepo/ui/\*\*/\* to generate CSS classes.
