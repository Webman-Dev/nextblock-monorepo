...
- **In Monorepo (Dev):** apps/nextblock imports libs/ui via **TypeScript Path Aliases** (defined in tsconfig.base.json).
  - _Example:_ import { Button } from '@nextblock-monorepo/ui' \-\> resolves to libs/ui/src/index.ts.
- **In Production (User):** The user's app imports libs/ui via **node_modules**.
  - _Example:_ import { Button } from '@nextblock-monorepo/ui' \-\> resolves to node_modules/@nextblock-monorepo/ui/dist/index.js.
...
- **Issue:** When installed as an NPM package, the file paths to SQL migrations inside node_modules/@nextblock-monorepo/db are different than in the monorepo.
...
- **Goal:** Ensure libs/ui is transpiled correctly. The consumer app's tailwind.config.js must effectively scan node_modules/@nextblock-monorepo/ui/\*\*/\* to generate CSS classes.
